<?php echo $header; ?>
<script>
  var curnUrl = window.location.href;
  if (curnUrl.indexOf("ne-orhidei") > -1){
    /*$(document).ready(function(){
          $(".sf-availability").css("display","none");
    })*/
    $("#main-menu-item-6").css("background","rgb(68, 67, 73)");
      $("#main-menu-item-6").find(".main-menu-text").css("color","rgb(255, 255, 255)");
  }
  if(curnUrl.indexOf("#/a") == -1 && curnUrl.indexOf("zakaz-orchidey") == -1 && curnUrl.indexOf("ne-orhidei") == -1){
    curUrl=curnUrl+"#/availability=1/sort=p.sort_order/order=ASC/limit=100";
     //$(".sf-availability").find(".box-content").children().children().children().children().first().attr('checked',true);
     //var chk=$(".sf-availability").find(".box-content").children().children().find(".sf-name").prev().prop("checked");
     //alert(chk);
    setLocation(curUrl);
  }
